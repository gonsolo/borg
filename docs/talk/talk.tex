...
\begin{frame}{Where the Toolchain Goes Next}
  \begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Faster build/debug loops for OpenROAD and LibreLane
          themselves.} A signoff run here takes hours; today alone,
          isolating one failing step with LibreLane's
          \texttt{create-reproducible}/\texttt{eject} tooling turned a
          4-hour re-run into a 2-minute standalone probe --- and surfaced a
          real, previously-unreported null-pointer crash in OpenROAD's
          clock-tree synthesis (\texttt{TritonCTS}) in the process. More of
          this: fast, debuggable iteration loops are as much a
          contribution as the tapeout itself.
  \end{itemize}
  \vfill
  \centering
